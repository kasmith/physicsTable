    ...
    def minimise ( self, theta_0, ens_size=2, report=500 ):
        ...
        while  (np.linalg.norm(theta_saved-theta)/np.linalg.norm(theta_saved) >\
            ...
            for j in np.arange ( ens_size ):
                ...
                if self.max_vals is not None: theta_plus = np.minimum ( theta_plus, self.max_vals )
                theta_minus = theta - ck*delta
                if self.min_vals is not None: theta_minus = np.maximum ( theta_minus, self.min_vals )
